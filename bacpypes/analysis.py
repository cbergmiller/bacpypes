...
def decode_ethernet(s):
    if DEBUG: _logger.debug("decode_ethernet %s...", _hexify(s[:14]))
...
def decode_vlan(s):
    if DEBUG: _logger.debug("decode_vlan %s...", _hexify(s[:4]))
...
def decode_ip(s):
    if DEBUG: _logger.debug("decode_ip %r", _hexify(s[:20]))
...
def decode_udp(s):
    if DEBUG: _logger.debug("decode_udp %s...", _hexify(s[:8]))
...
def decode_packet(data):
    ...
    if pdu.pduData[0] != '\x01':
        if DEBUG: _logger.debug("    - not a version 1 packet: %s...", _hexify(pdu.pduData[:30]))
        return None
    # it's an NPDU
    try:
        npdu = NPDU()
