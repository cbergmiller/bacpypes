...
class BitString(Atomic):
    ...
    def __init__(self, arg=None):
        ...
        if arg is None:
            ...
            all_ints = all_strings = True
            for elem in arg:
                all_ints = all_ints and ((elem == 0) or (elem == 1))
                all_strings = all_strings and elem in self.bitNames
            if all_ints:
                self.value = arg
            elif all_strings:
                for bit in arg:
    ...
    def is_valid(cls, arg):
        ...
        if isinstance(arg, list):
            ...
            for elem in arg:
                all_ints = all_ints and ((elem == 0) or (elem == 1))
                all_strings = all_strings and elem in cls.bitNames
            if all_ints or all_strings:
                return True
        return False
    ...
    def __getitem__(self, bit):
        if isinstance(bit, int):
            ...
            if not bit not in self.bitNames:
...
class Enumerated(Atomic):
    ...
    def is_valid(cls, arg):
        ...
        """
        return (isinstance(arg, int) and (arg >= 0)) or isinstance(arg, str)
...
class ObjectIdentifier(Atomic):
    ...
    def set_tuple(self, obj_type, obj_instance):
        # allow a type name as well as an integer
        if isinstance(obj_type, int):
            # try and make it pretty
            obj_type = self.objectTypeClass._xlate_table.get(obj_type, obj_type)
        elif isinstance(obj_type, str):
            # make sure the type is known
            if obj_type not in self.objectTypeClass._xlate_table:
                raise ValueError("unrecognized object type '%s'" % (obj_type,))
        else:
            raise TypeError("invalid datatype for objType: %r, %r" % (type(obj_type), obj_type))
        # check for valid instance number
        if (obj_instance < 0) or (obj_instance > ObjectIdentifier.maximum_instance_number):
            raise ValueError("instance number out of range")
        # pack the components together
        self.value = (obj_type, obj_instance)

    def get_tuple(self):
        """Return the unsigned integer tuple of the identifier."""
        obj_type, obj_instance = self.value
        if isinstance(obj_type, int):
            pass
        elif isinstance(obj_type, str):
            # turn it back into an integer
            obj_type = self.objectTypeClass()[obj_type]
        else:
    ...
    def set_long(self, value):
        # suck out the type
        obj_type = (value >> 22) & 0x03FF
        # try and make it pretty
        obj_type = self.objectTypeClass()[obj_type] or obj_type
        # suck out the instance
        obj_instance = value & 0x003FFFFF
        # save the result
        self.value = (obj_type, obj_instance)

    def get_long(self):
        """Return the unsigned integer representation of the identifier."""
        obj_type, obj_instance = self.get_tuple()
        # pack the components together
        return ((obj_type << 22) + obj_instance)
    ...
    def __str__(self):
        ...
        if isinstance(obj_type, str):
            ...
            typestr = "Bad %d" % (obj_type,)
        elif obj_type in self.objectTypeClass._xlate_table:
            typestr = self.objectTypeClass._xlate_table[obj_type]
        elif obj_type < 128:
            typestr = "Reserved %d" % (obj_type,)
        else:
            typestr = "Vendor %d" % (obj_type,)
        return "ObjectIdentifier(%s,%d)" % (typestr, obj_instance)
